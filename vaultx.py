...
try:
    from tpm_fingerprint_lib.tpm_ops import (
        TPMOperations,
    ...
    class _MissingTPMModule(Exception):
        pass

    TPMNotAvailableError = _MissingTPMModule
    SealingError = _MissingTPMModule
    UnsealingError = _MissingTPMModule
    PCRMismatchError = _MissingTPMModule
...
class VaultXApp:
    def __init__(self, root):
        self.root = root
        self.root.title("VAULTX - SECURE FILE VAULT")
        ...
        self.tpm_detection_error = None
        self.tpm_binding_var = None
        self.tpm_metadata_suffix = ".vx2.tpm"
        self.detect_tpm_capabilities()
    ...
    def setup_ui(self):
        ...
        # TPM Binding option
        self.create_tpm_binding_option()
    ...
    def detect_tpm_capabilities(self):
        if TPMOperations is None:
            self.tpm_available = False
            self.tpm_detection_error = "TPM dependency not installed"
            return

        try:
            self.tpm_ops = TPMOperations()
            self.tpm_available = self.tpm_ops.is_tpm_available()
            if not self.tpm_available:
                self.tpm_detection_error = "TPM hardware or drivers unavailable"
        except TPMNotAvailableError as exc:
            self.tpm_available = False
            self.tpm_detection_error = str(exc)
        except Exception as exc:
            self.tpm_available = False
            self.tpm_detection_error = f"TPM initialization failed: {exc}"

    def log_tpm_status(self):
        if not hasattr(self, 'log_lines'):
            return

        if self.tpm_available:
            self.log("TPM detected. TPM binding is optional for future encryptions.", self.colors['secondary'])
        else:
            reason = self.tpm_detection_error or "TPM unavailable"
            self.log(f"TPM binding disabled: {reason}", self.colors['warning'])
    ...
    def create_status_bar(self):
        ...
        tk.Label(status_frame, 
                text="AES-256-GCM-SIV · Argon2id · HKDF-SHA512",
                font=("Courier New", 9),
    ...
    def create_title(self):
        ...
        subtitle = tk.Label(title_frame, 
                           text="SECURE FILE VAULT // AES-256-GCM-SIV",
                           font=("Courier New", 10),
    ...
    def create_tpm_binding_option(self):
        option_frame = tk.Frame(self.main_frame, bg=self.colors['card'], relief=tk.RAISED, bd=1)
        option_frame.pack(fill=tk.X, padx=20, pady=5)
        
        tk.Label(option_frame, text="TPM Binding (Optional)", font=("Courier New", 10, "bold"),
                fg=self.colors['text'], bg=self.colors['card']).pack(anchor=tk.W, padx=10, pady=(10, 2))
        
        state = tk.NORMAL if self.tpm_available else tk.DISABLED
        self.tpm_binding_var = tk.BooleanVar(value=False)
        self.tpm_status_var = tk.StringVar(value=self._tpm_status_text())
        
        tpm_toggle = tk.Checkbutton(
            option_frame, text="Restrict decryption to this TPM (creates .vx2.tpm metadata)",
            variable=self.tpm_binding_var,
            font=("Courier New", 9),
            fg=self.colors['text'], bg=self.colors['card'],
            selectcolor=self.colors['panel'],
            activebackground=self.colors['card'],
            command=self.on_tpm_toggle,
            state=state
        )
        tpm_toggle.pack(anchor=tk.W, padx=10, pady=(0, 5))
        
        status_label = tk.Label(option_frame, textvariable=self.tpm_status_var,
                                font=("Courier New", 8), fg=self.colors['dim'], bg=self.colors['card'])
        status_label.pack(anchor=tk.W, padx=10, pady=(0, 10))

    def _tpm_status_text(self):
        if self.tpm_available:
            return "TPM ready. Toggle to protect files with hardware binding."
        reason = self.tpm_detection_error or "TPM not available on this system."
        return f"TPM unavailable: {reason}"

    def on_tpm_toggle(self):
        if self.tpm_binding_var.get():
            self.tpm_status_var.set("TPM binding enabled. A .vx2.tpm file will accompany the encrypted payload.")
        else:
            self.tpm_status_var.set(self._tpm_status_text())
    ...
    def create_warning_footer(self):
        ...
        warning_text = "⚠  LOSS OF PASSWORD = PERMANENT DATA LOSS  ·  OUTPUT SAVED ALONGSIDE SOURCE FILE"
...
- Use strong passwords (ELITE strength recommended)
- Output files saved alongside source files
    ...
    def load_recent_files(self):
        try:
            ...
            self.log(f"Unable to load recent files: {exc}", self.colors['warning'])
            self.recent_files = []
    ...
    def save_recent_files(self):
        try:
            ...
            self.log(f"Unable to save recent files: {exc}", self.colors['warning'])
    ...
    def update_recent_dropdown(self):
        ...
        if not self.recent_files:
            menu.add_command(label="Recent Files...", state=tk.DISABLED)
        else:
            for i, file_info in enumerate(self.recent_files):
                label = f"{file_info['name']} ({file_info['operation']})"
                menu.add_command(label=label, command=tk._setit(self.recent_var, i))
    ...
    def on_recent_file_selected(self, index):
        try:
            ...
            self.log(f"Unable to select recent file: {exc}", self.colors['warning'])
    ...
    def _tpm_metadata_path(self, encrypted_path):
        return f"{encrypted_path}{self.tpm_metadata_suffix}"
    ...
    def encrypt_file(self, password):
        ...
        should_bind_tpm = bool(self.tpm_binding_var.get()) if self.tpm_binding_var else False
        sealed_blob = None
        if should_bind_tpm:
            if not self.tpm_available or self.tpm_ops is None:
                raise ValueError("TPM binding requested but TPM support is unavailable.")
            try:
                sealed_blob = self.tpm_ops.seal_data(key)
                self.root.after(0, lambda: self.log("TPM metadata sealed for this encryption.", self.colors['secondary']))
            except SealingError as exc:
                raise ValueError(f"Failed to seal key with TPM: {exc}")
        ...
        metadata_path = self._tpm_metadata_path(output_path)
        if sealed_blob:
            try:
                with open(metadata_path, 'wb') as meta_file:
                    meta_file.write(sealed_blob)
                self.root.after(0, lambda: self.log("TPM metadata persisted (.vx2.tpm).", self.colors['secondary']))
            except OSError as meta_error:
                self.root.after(0, lambda: self.log(f"ERROR writing TPM metadata: {meta_error}", self.colors['danger']))
                raise
        else:
    ...
    def decrypt_file(self, password):
        ...
        sealed_blob = None
        metadata_path = self._tpm_metadata_path(self.selected_file)
        if os.path.exists(metadata_path):
            if not self.tpm_available or self.tpm_ops is None:
                raise ValueError("TPM binding metadata present but TPM support is unavailable.")
            with open(metadata_path, 'rb') as meta_file:
                sealed_blob = meta_file.read()
            try:
                unsealed_key = self.tpm_ops.unseal_data(sealed_blob)
            except (PCRMismatchError, UnsealingError, TPMNotAvailableError) as exc:
                raise ValueError(f"Failed to unseal TPM metadata: {exc}")
        else:
            unsealed_key = None
        ...
        if unsealed_key is not None:
            if not secrets.compare_digest(key, unsealed_key):
                raise ValueError("Provided password does not match TPM-protected metadata.")
            self.root.after(0, lambda: self.log("TPM metadata validated against the password.", self.colors['secondary']))
    ...
    def start_hex_animation(self):
        def animate_hex():
            if hasattr(self, 'hex_cells') and self.hex_cells:
                cell = secrets.choice(self.hex_cells)
               
                # Choose color
                color = secrets.choice([self.colors['hex_cyan'], self.colors['hex_green']])
